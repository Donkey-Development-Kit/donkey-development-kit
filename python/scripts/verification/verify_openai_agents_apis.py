...
def main() -> None:
    ...
    binding = MCPServerStreamableHttp(params=params, cache_tools_list=True, name="probe")
    assert isinstance(binding, MCPServerStreamableHttp)
    assert binding.params["url"] == URL and binding.params["headers"] == HEADERS
    ...
    emit(
        ["openai-agents", "openai", "mcp"],
        [describe(MCPServerStreamableHttp)],
        {"params": params, "cache_tools_list": True, "name": "probe"},
        {"name": tool.name, "description": tool.description, "params_json_schema": schema},
    )
