...
[do-not-build](/roadmap) list); it lets you **steer** the gateway's cache per
...
[`donkey.run(...)`](/identity), the controls bind to a context variable, so they
...
[`donkey.last_call.cache_status`](/reference/last-call#semantic-cache) /
`.cache_score` and the OTel span. The same
[degradation](/frameworks) as `donkey.run(...)` applies: a `connection_kwargs()`
