...
tokens. The [gateway identity example](/examples/general/gateway-identity) walks
through every field.
...
### Semantic caching & semantic routing <Badge tone="live">Live</Badge>
...
[`donkey.cache(...)`](/budget#semantic-cache-steering) — and reports what
happened:
...
advances the [token budget](/budget) — the replayed `usage` is not fresh spend.
