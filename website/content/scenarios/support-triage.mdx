---
description: Draft replies to a queue of support tickets, one carrying PII, with typed refusals, correlation IDs, and OTel spans — the shipped LangGraph demo, end to end, no gateway.
---
...
drafted reply against the [local simulator](/simulator) only — no Anypoint
credentials, no real gateway — exercising four governance pieces at once.

## What it demonstrates

- **Runs with no gateway.** The demo boots the [local simulator](/simulator)
...
- **The PII-masking branch actually executes.** The simulator runs the
  `pii_block:every=5` [scenario](/simulator#scenario-scripting) — every fifth
  ...
  LangGraph run as a typed [`PIIDetected`](/errors), not a framework-wrapped
  generic error.
- **Correlation reaches every node for free.** Each ticket runs inside a
  [`donkey.run(id=...)`](/telemetry) block; the run id shows up in a graph
...
- **Every governed call emits an OTel span** — refusals included — exported to a
  bundled local OTLP collector with [zero config](/telemetry): setting the
...
<Callout type="info">
  **Honest note on the block.** The simulator triggers on the request *count*
  ...
  [It replays; it does not evaluate](/simulator#it-replays-it-does-not-evaluate).
</Callout>
...
([Verification policy](/concepts/verification)). `ChatOpenAI` / `StateGraph` are
...
- [Nightly batch](/scenarios/nightly-batch) — the same governance, applied to an
  unattended overnight job that paces itself against a budget window.
- [Typed refusals](/errors) — the full exception taxonomy the block above lands
  in, with a per-exception "retryable?" cookbook.
- [Testing & conformance](/testing) — run this same agent factory through the
