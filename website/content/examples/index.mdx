---
description: Runnable DDK examples from the companion demos repo — narrative demos that run offline against the local simulator, and short OpenAI scripts for a live gateway.
---
...
<CardGrid>
  <Card title="Governed client" href="/examples/governed-client" icon="◆" badge="Offline" badgeTone="live">
    Stock client versus governed client, then `@donkey.governed` and `@donkey.tool`.
  </Card>
  <Card title="Typed refusals" href="/examples/typed-refusals" icon="⊘" badge="Offline" badgeTone="live">
    Every captured rejection shape through `classify()`, plus `GatewayUnavailable`.
  </Card>
  <Card title="Budget & pacing" href="/examples/budget-and-pacing" icon="◷" badge="Offline" badgeTone="live">
    The token window as an object; `pace(reserve=)` and `wait_for_reset()`.
  </Card>
  <Card title="Simulating refusals" href="/examples/simulating-refusals" icon="⟳" badge="Offline" badgeTone="live">
    Run your refusal branch with `donkey.simulate()` and `donkey mock --scenario`.
  </Card>
  <Card title="Conformance suite" href="/examples/conformance" icon="✓" badge="Offline" badgeTone="live">
    `pytest --donkey-conformance` grading a naive agent, then the fixed one.
  </Card>
  <Card title="Telemetry" href="/examples/telemetry" icon="≋" badge="Offline" badgeTone="live">
    GenAI spans, `donkey.run(id=…)` correlation, and zero-config OTLP.
  </Card>
  <Card title="Framework objects & model handles" href="/examples/framework-objects" icon="⚙" badge="Offline" badgeTone="live">
    Native framework objects, `connection_kwargs()`, and `resolve()`.
  </Card>
  <Card title="LangGraph agent" href="/examples/langgraph-agent" icon="↻" badge="Live gateway" badgeTone="accent">
    A real tool-calling loop, governed end to end.
  </Card>
  <Card title="Gateway identity (last_call)" href="/examples/gateway-identity" icon="⌖" badge="Offline" badgeTone="live">
