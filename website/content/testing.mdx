---
description: Test your agent's failure handling — in-process refusal injection, a pytest conformance suite you run against your own agent, and an out-of-process gateway fixture.
---
...
classifier and the [local simulator](/simulator) use, so your agent sees exactly
...
<Callout type="info">
  ...
  `set_scenarios(...)`, or [`donkey mock --scenario`](/simulator#scenario-scripting).
</Callout>
...
The `gateway` fixture boots the [local simulator](/simulator) on an ephemeral
...
- `gateway.set_scenarios(...)` arms [scenario scripting](/simulator#scenario-scripting)
...
| | [`donkey mock`](/simulator) | `gateway` fixture | `simulate()` |
|---|---|---|---|
