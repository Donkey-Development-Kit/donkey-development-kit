---
description: In-process refusal injection, a pytest plugin you run against your own agent, and an out-of-process gateway fixture.
---
...
<Callout type="info">
  **Phase 1 — all three ship.** In-process refusal injection
  ...
  for a subject that does not import the SDK. See [Roadmap](/roadmap) and
...
[local simulator](/simulator) on an **ephemeral port**, hands your test its
...
- `gateway.set_scenarios(...)` arms the same
  [scenario scripting](/simulator#scenario-scripting) (`pii_block`, `injection`,
