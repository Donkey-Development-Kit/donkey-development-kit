...
| --- | --- | --- |
| `.correlation_id` | The **run** id, shared by every call in a [`donkey.run()`](/telemetry#correlation-ids) block | The `X-Correlation-Id` request header the client sent — always equals what went on the wire. |
