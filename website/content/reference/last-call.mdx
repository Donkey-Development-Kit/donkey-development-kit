---
description: Every field on donkey.last_call — the observability status, the gateway identity, the routing and fallback signals, the per-call token usage, and the semantic-cache outcome the gateway reported about the most recent governed model call.
---
...
[`donkey.cache(...)`](/budget#semantic-cache-steering); read the outcome here.
...
| `cache_hit` | `bool` | `True` iff `cache_status == "hit"` — a verbatim replay with no provider round-trip. A hit never advances the [budget](/budget) (a replay is no fresh spend). |
