...
<Callout type="info">
  ...
  [Governance](/concepts/governance)). DDK doesn't use that form: it always
  sends the two-header pair, because `client_id` is the per-agent
  [attribution](/concepts/attribution) unit. The colon-joined value is not an
...
[`AuthProvider`](/identity) passed as `Donkey(llm_auth=…)`, so the SDK can
...
full value regardless. See [Verification policy](/concepts/verification).
