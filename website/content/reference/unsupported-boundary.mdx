...
The [Local Mode investigation](https://github.com/Donkey-Development-Kit/donkey-development-kit/blob/develop/docs/evidence/local-gateway/README.md)
records a registration prerequisite in Flex Gateway 1.14.0 and a real local
Injection Protection rejection. The tested stock image did not resolve the
recorded Model Proxy/MCP policy references. This is a version-specific finding;
a complete Local Mode Model Proxy or MCP Bridge remains unverified. Use DDK's
labelled simulator for the default development loop and opt into real gateway
tests with a dedicated registration artifact.
