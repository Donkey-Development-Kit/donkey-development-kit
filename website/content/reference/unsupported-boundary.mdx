...
Injection Protection was captured live on 2026-09-26 in **msaleme's own org /
Sandbox**, not the DDK team sandbox: instance `21199453`, policy
`injection-protection` `1.0.1`, gateway `1.12.9`. The response was HTTP `400`,
`x-injection-protection: blocked`, and a 79-byte JSON body identifying SQL
Injection in the request body. The [capture evidence](https://github.com/Donkey-Development-Kit/donkey-development-kit/blob/develop/docs/evidence/injection-protection-253/README.md)
records the prompt, exact bytes, versions and teardown. Unknown content-moderation
fall-through responses remain unverified; no such response was observed.
