---
description: OTel GenAI spans at a pinned semconv version, dual-emitted with a stable donkey.* namespace.
---
...
| `donkey.policy.type` | `donkey.*` (stable) | The refusal type slug (e.g. `token_budget`, `pii_detected`) — see [Errors](/errors). |
...
opt-in is set. See [Telemetry & cost](/telemetry) for the obligation you take on
by enabling it.
...
<Callout type="info">
  ...
  *and* reads the inbound `X-Correlation-Id` — a probe confirmed the response
  ...
  names ([Verification policy](/concepts/verification)).
</Callout>
...
- **Cost tags.** The fixed `team` / `project` / `env` / `enduser.id` dimensions
  ...
  shipped) — see [Telemetry & cost](/telemetry).
...
- [Errors](/errors) — the refusal taxonomy behind `donkey.policy.type`.
