---
description: Run a local server that replays real gateway rejections, so you can build and test failure handling without a governed gateway.
---
...
[`PIIDetected`](/errors), not a raw `403`.
...
[Budget & pacing](/budget) runs end-to-end against the simulator. For a real
...
Every fifth call raises a typed [`PIIDetected`](/errors) in your agent, so your
...
to a minute and your [pacing and resume logic](/budget) is exercised in ninety
seconds.
...
The request surfaces as a typed [`PromptInjectionBlocked`](/errors) (via the
...
- **Its fixtures are the SDK's test fixtures.** The simulator serves the same
  ...
  [error taxonomy](/errors) cannot drift apart. The wheel ships a sha256
...
- [Testing & conformance](/testing) — `simulate()` for in-process unit tests,
  and the `gateway` pytest fixture that runs this simulator on an ephemeral port.
- [Error taxonomy](/errors) — the typed exceptions each shape maps to.
- [CLI](/cli) — the full `donkey` command reference.
