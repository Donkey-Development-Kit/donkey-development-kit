---
description: Governed by the gateway, understood by your code. DDK brings MuleSoft Agent Fabric and Omni Gateway awareness into your agent framework — typed refusals, budgets, telemetry and simulation, with native framework objects.
---

import { Callout, Tabs } from 'nextra/components'
import { Card, CardGrid, Figure, Hero } from '../components'

<Hero
  eyebrow="Open-source SDK for MuleSoft Agent Fabric"
  ...
  media={
    <Figure
      src="/img/introducing-ddk.jpg"
      alt="Introducing DDK: one control point for all AI traffic, a proven MuleSoft API management stack, zero friction for developers, governed by Agent Fabric. A short Python script makes a governed call and prints the remaining token budget, next to an Agent Fabric usage dashboard showing token usage by model and provider."
      width={1800}
      height={1012}
      priority
    />
...
<CardGrid columns={2}>
  <Card title="One control point for all AI traffic" icon="◆">
  ...
  <Card title="A proven stack, not a new one" icon="▣">
  ...
  <Card title="Zero friction for developers" icon="⚡">
  ...
  <Card title="Governed by Agent Fabric" icon="◎">
...
<Callout type="info">
  **DDK is an open-source, community-driven project** (Apache-2.0). It is **not
  ...
  connects to. Meet the people behind it on the [Team](/community/team) page.
</Callout>
...
<Figure
  src="/img/gateway-sdk-architecture.jpg"
  alt="Architecture: the AI Control Plane (agent registry, cost control, gateway federation, govern and observe) is connected to the Omni Gateway, which fronts APIs, MCP, LLM and agent traffic with policies and enforcement — auth and identity, token budgets and rate limits, PII detection, content safety and guardrails, model routing and fallback, audit trails, contract drift and tool-poisoning checks — and manages upstream APIs, MCP servers, LLM providers and AI apps. DDK, inside the agent, makes governed calls and receives typed refusals, scans code and publishes assets to the control plane, serves and exposes A2A agents, and emits OpenTelemetry spans to the observability stack."
  width={1800}
  height={1012}
/>
...
<Figure
  src="/img/example-with-without-ddk.jpg"
  alt="LLM routing example. Before: a stock OpenAI client calls gpt-4o directly, with no governance (no centralized controls or policy enforcement), no visibility (no observability or usage tracking) and no audit (no logs or attribution). After: the same call made through DDK is governed, observable and insightful, and the Agent Fabric usage dashboard shows total, input and output tokens, usage by model, and usage per consumer such as openai and hr-agent."
  width={1800}
  height={1014}
/>
...
constructor code — every [framework page](/frameworks) shows exactly which
three.
...
<CardGrid>
  ...
  <Card title="Frameworks" href="/frameworks" icon="⚙">
    Install and quickstart for each of the eight supported frameworks.
  </Card>
  <Card title="Examples" href="/examples" icon="▶">
  ...
  <Card title="Contribute" href="/community/contribute" icon="♥">
