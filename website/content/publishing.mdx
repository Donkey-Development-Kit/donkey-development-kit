---
description: Derive a manifest and agent card from your code, then register them with Exchange / Agent Registry from CI — the Agent Scanner for your git repo.
---
...
[`@donkey.tool`](/cli#donkeytool), plus MCP server definitions and agent entry
...
- **`auto`** (the default) imports your module, locates the tool and agent
  ...
  import-safe; declare explicit entrypoints rather than scanning whole packages:
...
- [CLI & decorators](/cli) — mark tools with `@donkey.tool` today.
- [A2A agents](/a2a) — serve and expose the agent whose card you publish.
- [Tool access](/tool-access) — discover and bind published tools from Exchange.
