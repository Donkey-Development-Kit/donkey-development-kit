---
description: The @donkey.governed and @donkey.tool decorators, and the donkey CLI — init, doctor, mock, and test.
---
...
Runs a function inside a [`donkey.run()`](/telemetry) scope:
...
The same markers are what the planned [scanner](/publishing) and
[A2A](/a2a) agent-card generator read, so marking tools now carries forward.
...
Runs the [local simulator](/simulator), which replays captured gateway
...
| `--scenario` | none | Fault-injection rule, repeatable. See [Scenario scripting](/simulator#scenario-scripting). |
...
See [Testing & conformance](/testing) for what the suite checks.

## Planned commands <Badge tone="roadmap">Roadmap</Badge>

These commands are part of the [Roadmap](/roadmap) and are not available in the
CLI yet:

- `donkey scan` and `donkey publish` — derive a manifest and agent card from
  your code and register them with Exchange. See [Scan & publish](/publishing).
- `donkey serve`, `donkey expose`, and `donkey dev` — serve your agent over
  A2A and expose it through the gateway. See [A2A agents](/a2a).
