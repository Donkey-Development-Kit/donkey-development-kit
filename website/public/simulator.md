...
[`PIIDetected`](https://donkey-development-kit.github.io/donkey-development-kit/errors.md), not a raw `403`, and your masking logic runs in your
...
minute, so the pacing and resume logic from [Budget & pacing](https://donkey-development-kit.github.io/donkey-development-kit/budget.md) is tested
...
[`PromptInjectionBlocked`](https://donkey-development-kit.github.io/donkey-development-kit/errors.md) (via the `x-injection-protection: blocked`
...
[Budget & pacing](https://donkey-development-kit.github.io/donkey-development-kit/budget.md) runs end-to-end against the simulator instead of
...
flips something a test asserts on, a committed **integrity manifest** pins the
sha256 of every fixture the simulator serves. Its source copy lives at
`tests/fixtures/fixtures.lock`; wheel builds copy it beside the packaged
fixtures, so an installed wheel can verify that its replayed bytes still match
the committed manifest. A *benign* byte edit — reformatted JSON, an added field
— fails the lock loudly instead of silently drifting the replayed bytes. A
genuine re-capture is accepted by regenerating the source lock
...
- The fixtures are the *same files* the `classify()` contract tests load, so
  ...
  integrity manifest fails loudly on any byte edit that the "both fail together"
