...
|---|---|---|
| Non-OpenAI proxy **ingress Format** (Anthropic / Gemini) | Both native ingress Formats now **VERIFIED (LIVE)**: Anthropic Messages route (#304) and Gemini `:generateContent` route (#540); no `donkey.gemini` SDK adapter | The default data-plane path is **Format=OpenAI**. MuleSoft Model Proxy [documents](https://docs.mulesoft.com/general/model-proxy) three selectable ingress Formats (OpenAI / Anthropic / Gemini) fixed at proxy creation, and both non-OpenAI Formats are now live-confirmed. The **Anthropic**-native Messages route is **VERIFIED (LIVE)**: a `Format=Anthropic` proxy serves `POST /<base-path>/v1/messages` natively (200, native Anthropic body; OpenAI-shape `/chat/completions` → 404), captured in `tests/fixtures/anypoint/anthropic_inbound/` (#304). The **Gemini**-native ingress is likewise **VERIFIED (LIVE)**: a `Format=Gemini` proxy serves a native `POST …/models/<model>:generateContent` passthrough (200, native Gemini body; OpenAI-shape → 400), captured in `tests/fixtures/anypoint/gemini_inbound/` (#540). `donkey.anthropic.client()` points `AsyncAnthropic` at the native route — but it requires a `Format=Anthropic` proxy (default DDK proxies are `Format=OpenAI`). The SDK ships **no** `donkey.gemini` adapter — a native `google-genai` adapter is demand-driven (#244); Gemini and Claude models are also reachable as **upstream providers** behind an OpenAI-format ingress via model-based routing, which works today. Ingress Format is a property of the provisioned proxy, not an SDK setting (no `llm_proxy_format` field). See [Which wire format does your proxy speak?](https://donkey-development-kit.github.io/donkey-development-kit/frameworks.md#which-wire-format-does-your-proxy-speak). |
...
| Injection-protection / fall-through bodies | Not captured | The **regex prompt-guard** (`matched_patterns` list) and **content-safety** (`…-action: reject` header) bodies are now **live-captured** — Azure Content Safety 2026-09-22 (#253) and its **Bedrock Guardrails** sibling 2026-09-24 (#568). Still uncaptured: the distinct **Injection Protection** policy's body (typed by the `x-injection-protection: blocked` header; no such proxy is deployed) and the undiscriminated content-moderation fall-through to a generic `PolicyViolation` — pinned from the policy pages, and no verification row flips until a live round-trip confirms them. No invented discriminator — tracked in #253. |
