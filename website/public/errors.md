...
|---|---|---|---|
| `AuthError` | `401`, or `403` + `www-authenticate` | **No** — terminal. The async client refreshes its token and retries **once** on a `401`, then surfaces it. | Check the consumer `client_id` / `client_secret` pair and that the consumer is authorized in API Manager. |
...
(`await donkey.budget.wait_for_reset()`, then continue), and `ModelSubstituted`
reports that a call *succeeded* against a different model than requested (opt-in
via `on_model_substitution="raise"`). `ConfigError` is raised locally, pre-flight,
and reports every missing field at once — fix the config and re-run.
