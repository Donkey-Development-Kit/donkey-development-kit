...
immediately and an unconditional retry would spin. `ModelSubstituted` reports
that a call *succeeded* against a different model than requested (opt-in via
`on_model_substitution="raise"`). `ConfigError` is raised locally, pre-flight, and
reports every missing field at once — fix the config and re-run.
