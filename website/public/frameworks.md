...
| LlamaIndex | ✅ | ❌ | Receives `default_headers`, not the SDK's HTTP client; `donkey.last_call` is unavailable. `is_chat_model=True` is forced. |
...
| MS Agent Framework | ✅ | ❌ | Receives `default_headers`, not the SDK's HTTP client; `donkey.last_call` is unavailable. Class name/kwarg **unverified**. |
