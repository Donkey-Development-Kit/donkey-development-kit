# LlamaIndex installed-package verification

Date: 2026-09-26. Base commit: `ee56f8112f633b0368629376b59fcf5fc1f1f2d1`.
Python 3.11.16, macOS arm64. Issue #641.

## Scope and results

`llama-index-core==0.14.25`; `llama-index-llms-openai-like==0.8.0`; `llama-index-tools-mcp==0.4.8`; `mcp==1.28.1`.

- [signature.txt](signature.txt): unmodified `verify_frameworks.py --only llamaindex --emit-verified` stdout/stderr; exit 0.
...
```bash
python -m pip install -r ../docs/evidence/frameworks/llamaindex/requirements.txt
...
python scripts/verify_frameworks.py --only llamaindex --emit-verified
python scripts/verification/verify_llamaindex_apis.py
```
