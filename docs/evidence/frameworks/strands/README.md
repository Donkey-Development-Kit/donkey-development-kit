# Strands installed-package verification

Date: 2026-09-26. Base commit: `ee56f8112f633b0368629376b59fcf5fc1f1f2d1`.
Python 3.11.16, macOS arm64. Issue #637.

## Scope and results

`mcp==1.28.1`; `openai==2.54.0`; `strands-agents==1.57.1`.

- [signature.txt](signature.txt): unmodified `verify_frameworks.py --only strands --emit-verified` stdout/stderr; exit 0.
...
```bash
python -m pip install -r ../docs/evidence/frameworks/strands/requirements.txt
...
python scripts/verify_frameworks.py --only strands --emit-verified
python scripts/verification/verify_strands_apis.py
```
