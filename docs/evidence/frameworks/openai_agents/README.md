# OpenAI Agents SDK installed-package verification

Date: 2026-09-26. Base commit: `ee56f8112f633b0368629376b59fcf5fc1f1f2d1`.
Python 3.11.16, macOS arm64. Issue #638.

## Scope and results

`mcp==1.28.1`; `openai==2.54.0`; `openai-agents==0.20.0`.

- [signature.txt](signature.txt): unmodified `verify_frameworks.py --only openai_agents --emit-verified` stdout/stderr; exit 0.
...
```bash
python -m pip install -r ../docs/evidence/frameworks/openai_agents/requirements.txt
...
python scripts/verify_frameworks.py --only openai_agents --emit-verified
python scripts/verification/verify_openai_agents_apis.py
```
