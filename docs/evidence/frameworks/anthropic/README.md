# Anthropic SDK installed-package verification

Date: 2026-09-26. Base commit: `ee56f8112f633b0368629376b59fcf5fc1f1f2d1`.
Python 3.11.16, macOS arm64. Issue #639.

## Scope and results

`anthropic==0.116.0`; `httpx==0.28.1`.

- [signature.txt](signature.txt): unmodified `verify_frameworks.py --only anthropic --emit-verified` stdout/stderr; exit 0.
...
No remote MCP connection, gateway round-trip, or conformance promotion is established. DDK binding and descriptor guards remain intact. The MCP field is an SDK request to Anthropic’s server-side connector, not a client-side MCP session. The MockTransport response in the probe is explicitly synthetic; it is not an Anypoint capture.
...
```bash
python -m pip install -r ../docs/evidence/frameworks/anthropic/requirements.txt
...
python scripts/verify_frameworks.py --only anthropic --emit-verified
python scripts/verification/verify_anthropic_apis.py
```
