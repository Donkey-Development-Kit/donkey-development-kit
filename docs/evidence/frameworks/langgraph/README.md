# LangGraph installed-package verification

Date: 2026-09-26. Base commit: `ee56f8112f633b0368629376b59fcf5fc1f1f2d1`.
Python 3.11.16, macOS arm64. Issue #642.

## Scope and results

`langchain-core==1.6.5`; `langchain-mcp-adapters==0.3.2`; `langchain-openai==1.6.6`; `langgraph==1.2.12`; `mcp==1.28.1`.

- [signature.txt](signature.txt): unmodified `verify_frameworks.py --only langgraph --emit-verified` stdout/stderr; exit 0.
...
```bash
python -m pip install -r ../docs/evidence/frameworks/langgraph/requirements.txt
...
python scripts/verify_frameworks.py --only langgraph --emit-verified
python scripts/verification/verify_langgraph_apis.py
```
