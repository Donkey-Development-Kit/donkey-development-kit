...
| Ingress **Format** (wire protocol the proxy accepts) — Anthropic-native route (#304) | `integrations/anthropic.py`, `llm/client.py` | UNVERIFIED (live) — **but documented** | Model Proxy supports **three selectable ingress Formats**, fixed at creation and unchangeable: **OpenAI** (works with all providers; multi-routing + fallback), **Gemini** (native Gemini format; single-provider), and **Anthropic** (native Anthropic API format for direct Claude requests; single-provider, no multi-routing/fallback). So the Anthropic-native route **exists as a product capability** — correcting the earlier "OpenAI-only, native route maybe-absent" assumption. Caveats keeping this UNVERIFIED for the SDK: (a) it must be *provisioned* that way — every DDK proxy is `Format=OpenAI`, so `donkey.anthropic.client()` pointed at them reaches Claude only as an *upstream provider*, not natively; (b) the **exact Anthropic ingress path** (docs show `/chat/completions` + `/responses` for OpenAI but do not state the Anthropic path — likely `/v1/messages`) and its **live behavior** are not yet captured against a real `Format=Anthropic` proxy. Per §12.8 (LLM data-plane, category 3) this stays behind the adapter's one-time warning until a live capture. Live capture blocked on provisioning a `Format=Anthropic` proxy (#304). The **Gemini** Format is now live-verified — see the next row. | 2026-09-23 | `docs.mulesoft.com/general/model-proxy` ("Supported Endpoint Formats"); live control-plane read of `ddk-bedrock-anthropic-model-routing` (instance `21192625`, DDK/Sandbox) confirming it is `Format=OpenAI`/model-based |
...
| Anthropic SDK | `anthropic.AsyncAnthropic(base_url, api_key, default_headers, http_client)` — model id per-call; the proxy's Anthropic-native ingress **Format is documented** but its exact path + live behavior are UNVERIFIED (see §2, #304) | UNVERIFIED | — | — | — |
